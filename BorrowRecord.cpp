...
bool BorrowRecord::collectFine(double amount) {
    ...
    // 检查支付金额是否超过应缴罚款
    if (paidFine + amount > totalFine + 0.01) { // 允许0.01的浮点误差
        return false;
    }
    ...
    // 更新罚款状态
    if (paidFine >= totalFine - 0.01) { // 允许0.01的浮点误差
        paidFine = totalFine;
        ...
        qDebug() << "[BorrowRecord] 罚款已支付 - 读者ID:" << readerID
                 << ", ISBN:" << ISBN << ", 金额:" << amount;
    } else {
        // 部分支付，状态仍为未支付
        qDebug() << "[BorrowRecord] 罚款部分支付 - 读者ID:" << readerID
                 << ", ISBN:" << ISBN << ", 金额:" << amount
                 << ", 已支付:" << paidFine;
    }
...
bool BorrowRecord::waiveFine(double amount) {
    ...
    if (amount <= 0) {
        ...
        qDebug() << "[BorrowRecord] 罚款已全额减免 - 读者ID:" << readerID
                 << ", ISBN:" << ISBN << ", 减免金额:" << totalFine;
    } else {
        // 部分减免
        ...
        qDebug() << "[BorrowRecord] 罚款部分减免 - 读者ID:" << readerID
                 << ", ISBN:" << ISBN << ", 减免金额:" << amount;
    }
