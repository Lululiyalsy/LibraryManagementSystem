...
void AdminWindow::setupBorrowTable()
{
    ...
    // （创建续借状态下拉框）：创建续借状态查找下拉框
    borrowRenewCombo = new QComboBox(borrowOperationWidget);
    ...
    borrowOperationLayout->addWidget(borrowRenewCombo);
    ...
    // （设置表头）：设置借阅表格表头
    borrowTable->setHorizontalHeaderLabels({"ISBN", "书名", "读者ID", "读者姓名", "借阅时间", "应还时间", "状态", "续借状态", "续借次数", "罚款金额", "已支付罚款"});
    // （设置列调整模式）：列自动拉伸填充
...
void AdminWindow::displayBorrowRecords(const std::vector<BorrowRecord> &records)
{
    ...
    for (const auto &record : records)
    {
        ...
        borrowTable->setItem(row, 6, new QTableWidgetItem(status));
        ...
        borrowTable->setItem(row, 7, new QTableWidgetItem(renewStatus));

        borrowTable->setItem(row, 8, new QTableWidgetItem(QString::number(record.getRenewCount())));
...
void AdminWindow::onBorrowRenewAudit()
{
    ...
    QString status = borrowTable->item(currentRow, 7)->text();
...
void AdminWindow::setupMessageWidget()
{
    ...
    messageTimeEdit->setFixedWidth(130);
...
void AdminWindow::onSearchMessage()
{
    ...
    QString readerId = messageReaderIdEdit->text().trimmed();
    ...
    for (const auto &msg : allMessages)
    {
        ...
        // 消息内容匹配
        if (match && !content.isEmpty() && !fields[3].contains(content, Qt::CaseInsensitive))
        {
            match = false;
        }
        // 消息状态匹配
        if (match && !status.isEmpty() && !fields[4].contains(status, Qt::CaseInsensitive))
        {
            match = false;
        }
